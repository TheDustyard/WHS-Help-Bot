...
fn leave(ctx: &mut Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    match class_id {
        Ok(class_id) => {
            ...
            {
                Ok(_) => match msg.member(&ctx.cache).unwrap().remove_role(&ctx, class_id) {
                    Ok(_) => msg.channel_id.say(
                        &ctx,
                        format!("Successfully left class `{}`", class_id.to_string()),
                    )?,
        ...
        Err(_) => {
                ...
                .next()
            {
                Some(class) => match msg
                    .member(&ctx.cache)
                    .unwrap()
                    .remove_role(&ctx, class.parse_role_id())
                {
                    Ok(_) => msg
                        .channel_id
                        .say(&ctx, format!("Successfully left class `{}`", class_name))?,
                    Err(e) => msg.channel_id.say(
                        &ctx,
                        format!("Failed to left class `{}`\n```{:?}```", class_name, e),
                    )?,
                },
                None => msg.channel_id.say(
                    &ctx,
                    format!("Could not leave class with name `{}`", class_name),
                )?,
            };
        }
    }
